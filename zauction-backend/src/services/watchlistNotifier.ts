...
import { getWishlistMessageByIndex, WHATSAPP_MESSAGES } from './whatsappMessages';

type WatchlistUpdateRow = {
    user_id: string;
    lot_id: string;
    current_bid: string | number | null;
    phone: string | null;
    last_bid: string | number | null;
    last_template_index: number | null;
};
...
async function getWatchlistUpdates(): Promise<WatchlistUpdateRow[]> {
...
async function updateUserState(userId: string, templateIndex: number) {
    await pool.query(
        `INSERT INTO watchlist_user_notification_state (user_id, last_template_index, last_sent_at)
         VALUES ($1, $2, NOW())
         ON CONFLICT (user_id)
         DO UPDATE SET last_template_index = EXCLUDED.last_template_index, last_sent_at = EXCLUDED.last_sent_at`,
        [userId, templateIndex]
    );
}

async function updateLotState(userId: string, lotId: string, bidValue: number) {
    await pool.query(
        `INSERT INTO watchlist_notification_state (user_id, lot_id, last_bid, updated_at)
         VALUES ($1, $2, $3, NOW())
         ON CONFLICT (user_id, lot_id)
         DO UPDATE SET last_bid = EXCLUDED.last_bid, updated_at = EXCLUDED.updated_at`,
        [userId, lotId, bidValue]
    );
}
...
async function runWatchlistCycle() {
    try {
        ...
        const updatesByUser = new Map<string, WatchlistUpdateRow[]>();
        ...
        for (const [userId, userUpdates] of updatesByUser.entries()) {
            const totalMessages = WHATSAPP_MESSAGES.wishlistUpdates.length;
            if (!totalMessages) {
                return;
            }
            ...
            const message = getWishlistMessageByIndex(nextIndex);
...
export function startWatchlistNotifier() {
